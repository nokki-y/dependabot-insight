...
│                      │
│  package.json        │──┐
│  package-lock.json   │  │  静的解析
│  tsconfig.json       │  │  （GitHub Actions ランナー上で実行）
│  src/**/*.ts         │  │
└──────────────────────┘  │
                          ▼
              ┌────────────────────────┐
...
- ソースコードファイル（AST 解析のために読み取られるが、外部には送信されない）
- `package-lock.json` の内容（依存ツリー解析のためにローカルで解析）
- `tsconfig.json` の内容（パスエイリアス解決のためにローカルで解析）
...
プライベートリポジトリでこの Action を使用する場合、以下の情報が外部に公開されることに注意してください。
