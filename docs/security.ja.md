...
| 送信先 | 送信する情報 | 返却される情報 |
|--------|------------|--------------|
| PRコメント | 影響サマリー（パッケージ名、ファイルパス、ルートパス、ファイル数） | — |
| Claude API（`anthropic-api-key` 設定時のみ） | 影響サマリー（PRコメントと同一内容）+ QAレポートの出力形式を指定するプロンプト | パッケージの必要性判断、テストケースと確認手順（= QAレポート。PRコメントとして投稿） |
